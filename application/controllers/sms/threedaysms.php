...
class Threedaysms extends CI_Controller{
    ...
    function index(){
        ...
        $threedaygroups = $this->groups_model->get_group_by_type(1);
        // print_r(json_encode($threedaygroups));
        //             return;

        //loop through each group picking the group contacts
        if ($threedaygroups) {
            foreach ($threedaygroups as $key => $value) {
