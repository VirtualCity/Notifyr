...
class Monthlysms extends CI_Controller{
    ...
    function index(){
        ...
        $weeklygroups = $this->groups_model->get_group_by_type(3);
        // print_r(json_encode($weeklygroups));
        //             return;

        //loop through each group picking the group contacts
        if ($weeklygroups) {
            foreach ($weeklygroups as $key => $value) {
