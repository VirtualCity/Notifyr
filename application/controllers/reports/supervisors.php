...
class Supervisors extends Admin_Controller
{
    ...
    function index(){
        ...
        $this->load->view('reports/email_supervisors',$data);
    }
    ...
    function mail(){
        ...
        if($townsList){
            ...
            foreach($townsList as $town){
                ...
                $query = $this->reports_m->get_purchase_report_by_town($town_name,$region_name);
                ...
                if($query){
                    ...
                    $tasList = $this->supervisors_m->get_supervisor_by_townID($town_id);
                    if($tasList){
                        //TAS present in town
                    ...
                    if($emailTo){
                        ...
                        if($file_created){
                           // echo 'success';
                            ...
                            $this->email->attach('./exports/supervisors/'.$town_name.'/purchase_report.xls');
                            $this->email->subject($town_name.' Purchases Report');
                            $this->email->message('Kindly find attached weekly report for purchases made in your town.');
    ...
    function createExcel($query,$town_name,$region_name){
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:J3')->applyFromArray($styleArray1);
        $objPHPExcel->getActiveSheet()->getStyle('A1:J3')->applyFromArray($styleArray12);
        $objPHPExcel->getActiveSheet()->getStyle('A4:J4')->applyFromArray($styleArrayHeaders);


        $objPHPExcel->getActiveSheet()->setCellValue('A1', 'Purchase_report-'.date('d-M-y'));
        ...
        foreach($query->result() as $data)
        {
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':J'.$row)->applyFromArray($styleArray1);
            $row++;
        }
